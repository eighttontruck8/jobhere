...
import { CREDENTIAL_PROFILE_ID } from "@/domain";
...
describe("ProfileForm", () => {
  it("저장된 프로필을 조회해 입력값과 자격증을 표시한다", async () => {
    ...
    expect(await screen.findByDisplayValue("870")).not.toBeNull();
  ...
  it("범위를 벗어난 값을 필드 오류로 표시하고 저장 요청하지 않는다", async () => {
    ...
    fireEvent.change(screen.getByLabelText("어학 점수"), {
      target: { value: "991" },
    });
    ...
    expect(
      screen.getByText("어학 점수는 0~990 범위의 정수여야 합니다."),
    ).not.toBeNull();
    expect(
      screen.getByText("한국사 등급은 1~6 범위의 정수여야 합니다."),
    ).not.toBeNull();
