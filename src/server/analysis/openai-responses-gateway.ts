...
[전형순서]는 전형 단계를 한 줄에 하나씩 '- 서류 M/D(요일)-M/D(요일)' 또는 '- 1차면접 M/D(요일)|📍대구'처럼 간결하게 작성하세요.
연도와 '날짜:', '장소:' 문구는 표시하지 마세요. 실제 장소는 '|📍지명', 온라인·화상 전형은 '|💻비대면'으로 표시하세요.
전형 명칭은 서류·필기·인성검사·1차면접·2차면접·최종면접처럼 짧고 일관되게 작성하고, 날짜가 없으면 '미정'으로 표시하세요.
온라인 이의제기, 채용 검증, 검증위원회, 심사위원회, 합격자 발표, 증빙서류 제출·등록 같은 행정 절차는 [전형순서]와 다른 섹션에도 넣지 마세요.
마감일을 다른 일정 날짜로 사용하거나 원문에 없는 날짜·장소를 추측하지 마세요.
