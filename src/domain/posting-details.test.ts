...
- 서류전형 (날짜: 미정, 장소: 비대면) -> 면접전형 (날짜: 추후 공지, 장소: 대구)
    `)).toEqual([
      ...
      { title: "전형순서", items: ["서류 미정 / 💻비대면", "면접 추후 공지 / 📍대구"] },
    ]);
  });
  ...
  it("기존 문단형 상세 내용도 항목별 섹션과 전형 흐름으로 변환한다", () => {
    ...
    expect(result.find(({ title }) => title === "전형순서")?.items).toEqual([
      "서류 미정 / 📍추후 공지",
      "필기 미정 / 📍추후 공지",
      "면접 미정 / 📍추후 공지",
    ]);
...
- 선택적 복지 제도
  ...
  it("긴 전형 문장을 짧은 일정으로 나누고 행정 절차를 제외한다", () => {
    const result = parsePostingDetails(`
[전형순서]
- 서류전형 (날짜: 2026년 9월 1일~9월 15일, 장소: 비대면) -> 온라인 이의제기/검증 (날짜: 2026년 10월 2일, 장소: 비대면) -> 1차면접 (날짜: 2026년 10월 20일, 장소: 대구) -> 채용검증위원회 (날짜: 2026년 12월 1일, 장소: 비대면) -> 최종면접 (날짜: 2026년 11월 25일, 장소: 대구)
    `);
